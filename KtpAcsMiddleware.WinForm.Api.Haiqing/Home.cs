...
namespace KtpAcsMiddleware.WinForm
{
    ...
    public partial class Home : Skin_Color
    {
        ...
        {
            this.Text = $"建信开太平【{ConfigHelper.KtpLoginProjectName}】_hq";
        ...
        public static void Myping(dynamic dgr)
        {
            ...
            if (isConn)
            {
                ...
                try
                {
                    int count = new PanelBaseHq().GetPanelUserCount(ip);
                    if (count > 0)
                    {

                        //设备数量

                        dgr.Cells["panelCount"].Value = count;
                    }
                    string state = new PanelBaseHq().GetIsNetworkServiceTest(ip);
                    dgr.Cells[8].Value = state;
