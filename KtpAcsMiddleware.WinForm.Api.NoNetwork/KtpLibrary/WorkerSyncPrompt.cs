...
namespace KtpAcsMiddleware.WinForm.Api.KtpLibrary
{
    public partial class WorkerSyncPrompt : FormBaseUi
    {
        ...
        private string workerSyn()
        {
            ...
            {
                //ipCount++;
                //Thread PaneThread = new Thread(new ParameterizedThreadStart(AddPaneIpInfo));
                //PaneThread.IsBackground = true; //关闭窗体继续执行
                //PaneThread.Start(ip);
                //if (ipCount == _ip.Count())
                //{
                //    PaneThread.Join();
                //}
        ...
        public void AddPaneIpInfo(dynamic ip)
        {
            ...
            List<Personlist> personlist = paneSummary.ResponseData;
