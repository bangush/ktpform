...
namespace KtpAcsMiddleware.WinForm.TeamWorkers
{
    public partial class AddWorkerInfo : Skin_Color
    {
        ...
        private bool _isKtp = false;
        private string ktpMag = "";
        WorkerAddState _workerAddState;
        public AddWorkerInfo(int teamType, bool isKtp = true, string selectedValue = null)
        {
            _isKtp = isKtp;
            InitializeComponent();
            SetformSize();
            ...
            BindNationsCb();
            //班组
            BindTeamsCb(teamType, selectedValue);
            //银行
            BindBankCardTypesCb();
            this.teamType = teamType;
        }

        public AddWorkerInfo(int workerId, bool isEdit, int teamType, int teamId, int recordId, bool isSys = false, bool isKtp = true)
        {
            ...
            this.teamType = teamType;
            _fromReader = isEdit;
            BindWorkerDetailed(workerId, isEdit, teamType, teamId, recordId, isKtp);
        }
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //if (WorkSysFail.list.Count > 0 && _isSys)
                //{//同步失败重新编辑添加

                //    var work = WorkSysFail.list.FirstOrDefault(a => a.userId == workers.userId.ToString());
                //    if (work != null)
                //        WorkSysFail.list.Remove(work);
                //}
                _send = "";
            ...
            try
            {
                ...
                { //编辑
                    ...
                    {//离线版本提交
                        ...
                        {
                            workers.isSyn = true;
                            workers.localTeamId = (int)TeamCb.SelectedValue;
                            workers.certificationStatus = 2;
                            workers.ktpMag = "";
                            workers.poId = workers.localTeamId;
                            workers.userId = workers.localUserId;
                            string save = new WorkerInfo().EditWorker(workers);
                            if (save == "")
                                WorkSysFail.dicWorkadd.Add(true, "添加成功");
                            else
                                WorkSysFail.dicWorkadd.Add(false, "添加失败:" + save);
                        }
                    }
                    return;
                }
                ...
                if (ConfigHelper.KtpUploadNetWork)
                {
                    try
                    {
                        ...
                        if (WorkSysFail.workAdd.Count() > 0 || !ConfigHelper.IsDivceAdd)
                        {
                ...
                {
                    workers.certificationStatus = 1;
                    workers.localTeamId = (int)TeamCb.SelectedValue;
                    if (workers.userId == null)
                    {
                        workers.createTime = DateTime.Now;

                        string save = new WorkerInfo().AddWorker(workers);
                        if (save == "")
                            WorkSysFail.dicWorkadd.Add(true, "添加成功");
                        else
                            WorkSysFail.dicWorkadd.Add(false, "添加失败:" + save);
